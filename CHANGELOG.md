...
- **DM-equivalent 高速パスの短文ack例外**: react 判定が最も刺さる状況（Ryoko の返信直後の「ありがとう」「了解」）は、bot が返信した時点でその会話が DM-equivalent 化してトリアージを恒久スキップするため、**構造的に react に到達できなかった**（実測: 30日間で skip 333件 vs トリアージ実行 248件、フルエンジン側の disposition trailer は導入以来 0 件使用）。短いメッセージ（30文字以下・疑問符/URL/メンション/添付なし）は DM-equivalent でもトリアージに回すように。
