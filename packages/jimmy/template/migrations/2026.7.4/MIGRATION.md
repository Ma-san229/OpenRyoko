# Migration: 2026.7.4 (GPT-5.6 対応 / Codex 既定モデル gpt-5.6 / モデル選択 UI 刷新)
...
`ryoko update` / `ryoko migrate --auto` が config-patch を自動適用し、`jinn.version` を `"2026.7.4"` に更新する。手動操作は不要。
